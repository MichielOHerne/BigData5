...
## search pattern for keywords in the tweets

search_vars = ["search_var", "search_var2", "search_var3", "search_var4", "search_var5"]
pattern = [r'\b%s\b' % re.escape(s.strip()) for s in keyword.lower()]
pattern2 = re.compile('|'.join(pattern))

for i in twitterStream:
