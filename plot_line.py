...
def plot_line(data_list, manom=5):
    ''' Returns an interactive line plot in the directory "Plots/"
        ...
        Optional argument for the Moving Average Number Of Messages manom= (int) '''
    x_axis = []
    y_axis = []
    x_moving_avg = []
    y_moving_data = []
    ...
    # Moving average
    y_moving_avg = []
    for i in range(manom, len(x_axis)):
        x_moving_avg.append(x_axis[i])
        sum = 0
        for j in range(0, manom):
            sum += y_axis[i-j]
        y_moving_avg.append(round(sum/manom, 2))
    ...
    mov_avg = Scatter(x=x_moving_avg, y=y_moving_avg, text=['Moving average']*len(x_moving_avg), line=dict(shape='spline', color='rgb(24,192,24)'), name='Moving average (last ' + str(manom) + ')', hoverinfo='y+text')
